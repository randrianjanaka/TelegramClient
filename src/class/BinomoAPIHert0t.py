...
class Client:
    def __init__(self):
        ...
        for i in self.assetList:
            if i["name"] == self.currency: self.assetId = i["id"]
            self.assetRic = i["ric"]
    ...
    def getCurrentBalance(self):
        headers = {'device-id': self.deviceId, 'device-type': 'web', 'authorization-token': self.authToken,
                   'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/97.0.4692.71 Safari/537.36'}
    ...
    def getHistoryMarket(self):
        ...
        return requests.get(
            "https://api.binomo.com/platform/candles/" + urllib.parse.quote_plus(self.assetRic) + "/" + str(
                now) + "T00:00:00/60?locale=en", headers=self.headers).json()["data"]
    ...
    def getBid(self, status, amount):
        ...
        self.sendWs('{"topic":"base","event":"create_deal","payload":{"amount":' + str(
            amount * 100) + ',"asset":"' + self.assetRic + '","asset_id":' + str(
            self.assetId) + ',"asset_name":"' + self.currency + '","created_at":' + str(
            int(time.time())) + ',"currency_iso":"IDR","deal_type":"' + self.walletType + '","expire_at":' + bidTime + ',"option_type":"turbo","tournament_id":null,"trend":"' + status + '","is_state":false},"ref":"~~","join_ref":"~~"}')
    ...
    def hook(self):
        while True:
            ...
            if data["event"] == "deal_created":
                ...
                print("[ CALLBACK OPERATION ] ASSET RATE CHANGED: " + str(
                    data["payload"]["trading_tools_settings"]["standard"]["payment_rate_standard"]) + "%")
                if data["payload"]["trading_tools_settings"]["standard"]["payment_rate_standard"] < 75:
                    self.stop = True
                else:
